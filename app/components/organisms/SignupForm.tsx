...
export default function SignupForm({ onSubmit, onLogin, loading = false }: Props) {
    ...
    const [selectedAvatar, setSelectedAvatar] = useState(isAvatarEnabled ? '' : AVAILABLE_AVATARS[0])
    ...
    const isFormValid = isNonEmpty(nickname) && isValidEmail(email) && isValidPassword(password) && passwordsMatch && isAvatarValid
    ...
    const formItems = [
        { id: 'nickname', component: <InputField label='Nickname' value={nickname} onChangeText={setNickname} placeholder='Your nickname' leftIconName='person-outline' error={nicknameError} /> },
        { id: 'email', component: <InputField label='Email Address' value={email} onChangeText={setEmail} error={emailError} /> },
        { id: 'password', component: <PasswordField label='Password' value={password} onChangeText={setPassword} visible={showPassword} onToggleVisible={() => setShowPassword(!showPassword)} error={passwordError} /> },
        { id: 'confirmPassword', component: <PasswordField label='Confirm Password' value={confirmPassword} onChangeText={setConfirmPassword} visible={showConfirm} onToggleVisible={() => setShowConfirm(!showConfirm)} error={confirmError} /> },
        ...(isAvatarEnabled ? [{ id: 'avatar', component: <AvatarSelector selectedAvatar={selectedAvatar} onAvatarSelect={setSelectedAvatar} /> }] : []),
        { id: 'submit', component: <PrimaryButton title='Create account' onPress={() => onSubmit({ userName: nickname, nickname, email, password, avatar: selectedAvatar })} disabled={!isFormValid || loading} loading={loading} /> },
        { id: 'accountPrompt', component: <AccountPromptRow text='Already have an account?' cta='Login' onPress={onLogin} /> },
