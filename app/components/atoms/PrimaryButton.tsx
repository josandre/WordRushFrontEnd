...
export default function PrimaryButton({ title, onPress, textStyle, disabled, loading }: PrimaryButtonProps) {
    return (
        <TouchableOpacity onPress={onPress} disabled={disabled || loading} style={[style.btn, style.container, webButtonStyles, getWebPointerStyles(), (disabled || loading) ? { opacity: 0.5 } : null]}> 
            {loading ? (
